...
(function($) {
    $(document).ready(function() {
        ...
        $('#cariproduk,#cariproduk-resp').attr('autocomplete', 'off');
        $('#cariproduk').autocomplete({
            source: base_url + "ajax/psearch",
            open: function(event, ui) {
                var q = $('#cariproduk').val().replace(/\+/g,"%2B").replace(/\s/g,'+');
                $('.ui-autocomplete').append('<li class="ui-menu-item"><a href="' + base_url + 'search/?query='
                                                + q +
                                             '&sort=4&page=1&limit=20"><div class="list_item_container"><center><div class="label-name-resp table-cell"><i class="fa fa-search"></i> Lihat Semua Hasil Pencarian <i class="fa fa-angle-double-right"></i></div><center></div></a></li>');
            }
        }).data( 'ui-autocomplete' )._renderItem = function( ul, item ) {
            var inner_html = '<a href="' + item.link + '"><div class="list_item_container"><div class="image"><img src="' + item.image + '"></div><div class="label-name">' + item.label + '</div></div></a>';
            return $( '<li></li>' )
        ...
        $('#cariproduk-resp').autocomplete({
            ...
            var inner_html = '<a href="' + item.link + '"><div class="list_item_container"><div class="label-name-resp">' + item.label + '</div></div></a>';
            return $( '<li></li>' )
                .data( 'item.autocomplete', item )
                .append(inner_html)
                .appendTo( ul );
