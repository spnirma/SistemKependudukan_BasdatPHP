...
class MailerNewsletter
{
    ...
    function save($data)
    {
        ...
        $CI->db->insert('mailer_newsletter',$insert); 
    ...
    function update($id, $data)
    {
        ...
        $CI->db->update('mailer_newsletter', $data);
        return $CI->db->affected_rows();
    }
    
    /* Get all new email */ 
    function getEmails($limit=5)
    {
        $CI =& get_instance();
        $now = date('Y-m-d H:i:s');
        $sql = "select * from mailer_newsletter where mailer_status='new' and mailer_created<'$now' order by mailer_created asc limit 0,$limit";
    ...
    {
        $CI =& get_instance();  
        $sql = "select * from mailer_newsletter where idmailer=". (int)$idmailer." limit 0,1";
