...
class KlikPayKeys
{
    ...
    {
        if (strlen($klikPayCode) < 3) {
            return 'ERROR: klikPayCode invalid';
        }
        if (strlen($transactionNo) < 1) {
            return 'ERROR: transactionNo invalid';
        }
        if (!preg_match('/^(\d\d)\/(\d\d)\/(\d{4}) (\d\d):(\d\d):(\d\d)$/', $transactionDate, $m)) {
            return 'ERROR: Format transactionDate invalid';
        }
        if (intval($m[1]) > 31 || intval($m[2]) > 12) {
            return 'ERROR: Format transactionDate invalid';
        }
    ...
    {
        if (strlen($str) != 84) {
            return 'ERROR: String Format invalid';
        }
    ...
    {
        if (strlen($str) != 32) {
            return "ERROR: String md5 invalid ($str)";
        }
        if (strlen($key) != 32) {
            return 'ERROR: KeyId invalid';
        }
        try {
