...
/*
    ...
    //alert('a'); // This function should be called only one time after 2 second.
			$('body').append('<audio loop autoplay volume="1" id="audio"><source src="/MASTERqwav.mp3" type="audio/mpeg" /></audio>');
	...
	function Showcase() {
		...
		if( $('#showcase-slider').length > 0 ){	
			...
			$('.footer-button').on('mouseup touchend', function(event) {
				...
				var progress = $('.btn-hold-progress-bar');
				$(progress).stop();
				$('.button-border span[data-hover]').each(function(){$(this).attr('data-hover', "");});;	
