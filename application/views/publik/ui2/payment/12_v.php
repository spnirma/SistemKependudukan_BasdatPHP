<h4>Detail Cicilan Kartu Kredit BCA</h4>
<div class="row">
    <div class="colo-md-3" style="float: left">
        <p style="margin-left: 10px">Pembayaran menggunakan Kartu Kredit BCA akan dilakukan pada halaman BCA.</p>
        <div style="margin-bottom: 50px">
            <span class="col-md-2" style="margin-top:0.5em;margin-left: -5px">Cicilan :</span>
            <div class="col-md-5" style="margin-left:-10px">
                <select id="id_product_cicilan12" name="id_product_cicilan12" class="form-control">
                    <option value="full">Full Payment</option>
                <?php if (!isset($getPaymentCicilanProduct[12])) : ?>
                    <?php if (empty($matrix_produk) && empty($matrix_plan)) : ?>
                        <?php if ($grandTotalAllMandiri > 1000000) : ?>
                            <?php foreach (unserialize(BCA_KARTU_KREDIT_SITE_ID_CICILAN) as $k => $v) { ?>
                                <option value="<?= $k ?>">Cicilan 0% <?= $k ?> bulan</option>
                            <?php } ?>
                        <?php endif; ?>
                    <?php endif; ?>
                <?php else: ?>
                    <?php foreach ($getPaymentCicilanProduct[12] as $v) { ?>
        ...
        <p style="margin-left: 10px">BCA Kartu Kredit bisa melakukan pembayaran menggunakan : </p>
...
<div class="row">
    <div class="colo-md-11" style="border: 3px solid #f3f1f2;padding: 0.5em;margin-left:5px;margin-top:1em">
