...
//        $this->load->model('homepage_config_m');
...
// redirect ('/comingsoon');die;
        ...
        $sql = 'SELECT * FROM penduduk  WHERE id_penduduk >0 
                order by id_penduduk desc
                ';
                // and jenis="'.$data['acara'].'"
        $q = $this->db->query($sql);
        $data['penduduk'] = $q->result();
//echo '<pre>';print_r($data['penduduk']);die;

//        $data['data']=$this->user_m->get_single('tbl_user', 'id_user', 1);

//        $data['banner_web'] = $this->home_ui2_m->getActiveBannerHomeVe('web');

		echo $this->load->render('frontend/main',$data); 
