...
if (!empty(get("jenis"))) $vJenis=get("jenis");
        ...
        $acara_count = $this->db
            ...
            if (!empty(get("s"))) {
                $acara_count = $acara_count->where("judul_acara like '%".get("s")."%'");
            }
            if (!empty(get("fr"))) {
                $acara_count = $acara_count->where("tgl >= '".get("fr")."'");
            }
            if (!empty(get("to"))) {
                $acara_count = $acara_count->where("tgl <= '".get("to")."'");
            }
        $acara_count = $acara_count->get("tbl_acara")->row();
        ...
        $this->load->view('admin/acara_v', $data);
    }
    ...
    public function form($id)
    {
        ...
        if (!empty($id)) {
            $data['data'] = $this->db->where("id", $id)->get("tbl_acara")->row_array();
        }

        if (!empty($this->input->post('submit'))) {
            $param = $this->input->post();
            $param['judul_acara'] = str_replace('\"','"',$param['judul_acara']);
            $param['tgl'] = date("Y-m-d", strtotime($param['tgl']));
            $param['acara_ev'] = EVENTID;
            unset($param['submit']);

            //main upload
            unset($param["banner_upload"]);
            $file_name = $this->uploadLogic("banner_upload");
            if (!empty($file_name)) {
                $param['banner'] = $file_name;
            } else {
                if (!empty($data['data'])) {
                    $param['banner'] = $data['data']['banner'];
                } else {
                    $param["banner"] = "";
                }
            }

            $param['youtube'] = youtube_link_to_code($param['youtube']);
            if (!empty($id)) {
                $this->db->update("tbl_acara", $param, "id = '$id'");
            } else {
                $param['created_at'] = date("Y-m-d H:i:s");
                $this->db->insert("tbl_acara", $param);
            }
            redirect(base_url("admin/acara") . "?jenis=" . $param['jenis']);
        }

        $this->load->view('admin/acara_form_v', $data);
    }
